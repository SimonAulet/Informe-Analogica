\hbadness=10000pt  % Ignora Underfull hbox
\hfuzz=10000pt     % Ignora Overfull hbox
...
El despliegue es realizado por un servomotor, donde su debido control nos garantiza la precisión en la activacion, y recorrido final (control PWM).
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[auto, node distance=1.5cm]
        ...
        \draw [-latex] (input) -- node {V DC} (oscilador);
        \draw [-latex] (oscilador) -- node {Seno} (comparador);
        \draw [-latex] (comparador) -- node {PWM} (servomotor);
        \draw [-latex] (servomotor) -- node {Posición} (output);
    \end{tikzpicture}
...
$\displaystyle A(s) = \frac{V_o(s)}{V_f(s)} = 1 + \frac{R_F}{R_1}$

R1 y Rf están asociadas a la parte de realimentación negativa del amplificador.

$$
\left(1 + \frac{R_F}{R_1}\right) \cdot \frac{RCs}{R^2 C^2 s^2 + 3RCs + 1} = 1
$$

si sustituimos $j\omega$ por $s$ en la ecuación obtenemos que:
$$
\left(1 + \frac{R_F}{R_1}\right) \cdot \frac{RC(j\omega)}{R^2 C^2 (j\omega))^2 + 3RC(j\omega) + 1} = 1
$$
...
\begin{figure}[H]
  ...
  \caption{Circuito oscilador sinusoidal (Puente Wien)\hfill}%
  \label{fig:oscilador sinusoidal}
\end{figure}

\subsubsection*{Comparador}
\begin{figure}[H]
  \includegraphics[max width=0.8\linewidth]{graphics/comp.png}
  \caption{Circuito comparador para conversión senoidal a PWM\hfill}
...
\begin{figure}[H]
  ...
  \caption{Prueba en osciloscopio del oscilador Wien\mbox{}}%
